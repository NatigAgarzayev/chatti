...
export default function CustomEventModal({ calendarEvent }: { calendarEvent: any }) {
    ...
    const updateEditEventId = useStore(state => state.updateEditEventId)
    ...
    const editEventHandler = async (id: string) => {
        updateEditEventId(id)
        updateEditEventModal(true)
    }

    return (
        <ul className='border border-gray-200 rounded-md shadow-md bg-white overflow-hidden dark:bg-gray-800 dark:border-gray-700'>
            ...
            <li className='flex items-center gap-2 py-1 px-4'>
                ...
                <span className='text-sm text-gray-500 dark:text-gray-200'>{calendarEvent.start} - {calendarEvent.end}</span>
            </li>
            <li className='flex items-center gap-2 py-1 px-4'>
                <div>
                ...
                <span className='text-md text-gray-500 dark:text-gray-200'>{calendarEvent.people.length !== 0 ? calendarEvent.people.join(', ') : 'No assigns'}</span>
            </li>
            <li className='flex items-center gap-2 py-1 px-4'>
                ...
                <span className='text-sm text-gray-500 dark:text-gray-200'>{calendarEvent.description}</span>
            </li>
            <li className='flex items-center gap-2 py-1 px-4 justify-end'>
                <button onClick={() => editEventHandler(calendarEvent.id)} className='flex items-center gap-2 text-blue-500 rounded-md p-2 bg-blue-500/10 hover:bg-blue-500/20 transition-all duration-300'>
