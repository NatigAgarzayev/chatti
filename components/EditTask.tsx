                ...
                >
                    <DialogPanel className="max-w-lg space-y-4 border bg-white dark:bg-gray-800 p-6 rounded-3xl w-[370px]">
                        ...
                        <form action={editTaskHandler} className='min-w-40 w-full flex flex-col gap-4'>
                            ...
                            <button disabled={editTaskLoading || getQuillLoading} className={`p-2 bg-indigo-400 text-gray-700 font-bold rounded-3xl dark:bg-gray-100 disabled:bg-indigo-200 ${(getQuillLoading || editTaskLoading) && "animate-pulse"} `} type='submit'>{editTaskLoading ? "Process..." : "Update"}</button>
                        </form>
                    </DialogPanel>
                </motion.div>
            </div>
